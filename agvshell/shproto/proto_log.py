...
class proto_log_condition(proto_interface):
    ...
    def length(self) -> int:
        return self.phead.length()+ self.start_time.length()+self.end_time.length()+self.vct_log_type.length()

    def serialize(self)->bytes:
        return self.phead.serialize() +self.start_time.serialize()\
...
class proto_logs_file_path(proto_interface):
    ...
    def length(self):
        return self.phead.length()+self.vct_log_file_name.length()

    def serialize(self):
        return  self.phead.serialize()+self.vct_log_file_name.serialize()
