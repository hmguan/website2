...
def write_browser_logger(data):
    ...
    if LOG_INFO == log_type:
        LOG('browser_logger').info('user_id:{0}, msg:{1}'.format(data.get('user_id'),data.get('msg')))
    elif LOG_DEBUG == data.get('type'):
        LOG('browser_logger').debug('user_id:{0}, msg:{1}'.format(data.get('user_id'),data.get('msg')))
    elif LOG_WARNING == data.get('type'):
        LOG('browser_logger').warning('user_id:{0}, msg:{1}'.format(data.get('user_id'),data.get('msg')))
    elif LOG_ERROR == data.get('type'):
        LOG('browser_logger').error('user_id:{0}, msg:{1}'.format(data.get('user_id'),data.get('msg')))
    elif LOG_FATAL == data.get('type'):
        LOG('browser_logger').fatal('user_id:{0}, msg:{1}'.format(data.get('user_id'),data.get('msg')))
